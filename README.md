...
- bank of token、259AI、codex for、pinai、ccvibe、Galaxy：读取登录后可见的 /api/v1/groups/available，需要对应站点账号令牌。分组名称不拘泥于 Plus、Pro 等固定叫法，接口返回的分组名会原样作为渠道进入榜单，模型归属由接口的 platform 字段决定。
